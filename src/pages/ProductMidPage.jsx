...
/* ── Subcategory products listing view ─────────────────────────── */
function SubcategoryView({ category, sub, categorySlug }) {
  return (
    <main>
      {/* Subcategory Hero */}
      <div className="subcat-hero">
        {(sub.image || category.image) && (
          <div className="subcat-hero__img-wrap">
            <img src={sub.image || category.image} alt={sub.name} className="subcat-hero__img" />
            <div className="subcat-hero__overlay" />
          </div>
        )}
        <div className="container subcat-hero__content">
          <nav className="subcat-hero__breadcrumb">
            <Link to="/">Home</Link>
            <span>›</span>
          ...
          <div className="subcat-hero__icon"><Icon name={sub.icon} size={36} /></div>
          <h1 className="subcat-hero__title">{sub.name}</h1>
          <p className="subcat-hero__desc">{sub.desc}</p>
          <div className="subcat-hero__stat">
            <strong>{sub.products.length}</strong> Products Available
          </div>
        </div>
      </div>
        ...
        <AnimSection as="div" className="container">
          <div className="pcat-grid">
            {sub.products.map((product, pi) => (
              <Link
                key={product.slug}
                to={`/products/${categorySlug}/${sub.slug}/${product.slug}`}
                className="pcat-card"
                data-anim="fade-up"
                data-delay={String((pi % 3) * 100)}
              >
                <div className="pcat-card__thumb">
                  {(sub.image || category.image) && (
                    <img src={sub.image || category.image} alt={product.name} loading="lazy" />
                  )}
...
/* ── Inline product detail view (flat category) ────────────────── */
function FlatProductDetailView({ category, product, categorySlug }) {
  ...
  // Related: up to 3 others from same category
    ...
    <main>
          ...
          <div className="pd-hero__content">
            <div className="pd-hero__left" data-anim="fade-up" data-delay="100">
              <div className="pd-hero__badge-row">
                <span className="pd-hero__cat">{category.shortName}</span>
              ...
              <div className="pd-hero__actions">
                <Link to="/contact" className="btn btn-primary" style={{ fontSize: "14px", padding: "14px 32px" }}>Request a Quote ›</Link>
                <Link to="/contact" className="btn btn-outline" style={{ fontSize: "14px", padding: "14px 32px" }}>Download Datasheet</Link>
              </div>
            </div>
      ...
      <section className="section">
        <AnimSection as="div" className="container">
          ...
          <h2 className="section-title" data-anim="fade-up" data-delay="100" style={{ marginTop: "12px", marginBottom: "40px" }}>What Makes This System<br />Stand Out</h2>
        ...
        <AnimSection as="div" className="container">
          <div className="pd-specs-layout">
            <div data-anim="fade-right">
              ...
              <h2 className="section-title" style={{ marginTop: "12px", marginBottom: "32px" }}>Typical Specifications</h2>
            ...
            <div className="pd-specs-side" data-anim="fade-left">
              <div className="pd-side-card">
                ...
                <ul className="pd-side-list">{product.applications.map(a => <li key={a}>{a}</li>)}</ul>
              </div>
              <div className="pd-side-card">
                <div className="pd-side-card__title">Regulatory Standards</div>
                <ul className="pd-side-list">{standards.map(s => <li key={s}>{s}</li>)}</ul>
              </div>
              <div className="pd-side-card">
                <div className="pd-side-card__title">Validation Support</div>
                <ul className="pd-side-list">{validation.map(v => <li key={v}>{v}</li>)}</ul>
              </div>
            </div>
          </div>
        </AnimSection>
      ...
      <section className="pd-enquiry-banner">
        ...
        <div className="container pd-enquiry-banner__inner">
          <div>
            <div className="tag" style={{ background: "rgba(0,242,254,0.15)", color: "var(--cyan)", marginBottom: "12px" }}>Get a Quote</div>
            <h2 className="pd-enquiry-banner__title">Ready to specify the {product.name}?</h2>
            <p className="pd-enquiry-banner__desc">Share your process details and our engineers will prepare a detailed technical proposal.</p>
          </div>
          <div className="pd-enquiry-banner__actions">
            <Link to="/contact" className="btn btn-primary" style={{ fontSize: "15px", padding: "16px 36px" }}>Request a Quote ›</Link>
            <Link to="/contact" className="btn btn-outline" style={{ fontSize: "15px", padding: "16px 36px" }}>Talk to an Engineer</Link>
          </div>
        </div>
      </section>
      ...
      {related.length > 0 && (
        <section className="section">
          <AnimSection as="div" className="container">
            <div className="tag" data-anim="fade-up">More from {category.shortName}</div>
            <h2 className="section-title" data-anim="fade-up" data-delay="100" style={{ marginTop: "12px", marginBottom: "40px" }}>Related Products</h2>
            <div className="pcat-grid">
              {related.map((rp, i) => (
                <Link key={rp.slug} to={`/products/${categorySlug}/${rp.slug}`} className="pcat-card"
                  data-anim="fade-up" data-delay={String(i * 100)}>
                  <div className="pcat-card__thumb">
                    {category.image && <img src={category.image} alt={rp.name} loading="lazy" />}
...
/* ── Main export ────────────────────────────────────────────────── */
export default function ProductMidPage() {
  const { categorySlug, slug2 } = useParams();
  const category = findCategory(categorySlug);
  if (!category) return <Navigate to="/products" replace />;

  // Is slug2 a subcategory?
  const sub = category.subcategories?.find(s => s.slug === slug2);
  if (sub) {
    return <SubcategoryView category={category} sub={sub} categorySlug={categorySlug} />;
  }

  // Is slug2 a product (flat category)?
  const res = findProductInCategory(category, slug2);
  if (res?.product) {
    return <FlatProductDetailView category={category} product={res.product} categorySlug={categorySlug} />;
  }
